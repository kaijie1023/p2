...
class EnsembleModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # print(f"Resnet Output: {res_out}")
        # print(f"EfficientNet Output: {efficient_out}")
        
        # Weighted sum of outputs
        ensemble_out = (
            norm_weights[0] * res_out + 
            norm_weights[1] * efficient_out
        )

        # print(f"Ensemble Output: {ensemble_out}")
...
def stratified_cross_validation(
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X_train_val, y_train_val)):
        ...
        # Validation f1-score to update weights
        res_f1 = MulticlassF1Score(num_classes=num_classes).to(device)
        eff_f1 = MulticlassF1Score(num_classes=num_classes).to(device)
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                for inputs, labels in val_loader:
                    ...
                    res_out, eff_out = model.get_model_outputs(inputs)
                    # Update validation f1-score        
                    res_f1.update(res_out, labels)
                    eff_f1.update(eff_out, labels)
            ...
            print(f"ResNet Validation F1-score: {res_f1.compute().item():.4f}")
            print(f"EfficientNet Validation F1-score: {eff_f1.compute().item():.4f}")
            res_f1.reset()
            eff_f1.reset()
